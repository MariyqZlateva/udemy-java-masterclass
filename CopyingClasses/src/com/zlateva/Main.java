...
record Person(String name, String dob, Person[] kids) {
    ...
    public Person(Person p) {
        this(p.name, p.dob, p.kids() == null ? null : Arrays.copyOf(p.kids(),
                p.kids().length));
    }
...
public class Main {
    public static void main(String[] args) {
        ...
        //       Person[] personsCopy = Arrays.copyOf(persons, persons.length);//shallowCopy
//        Person[] personsCopy = new Person[5];
//        Arrays.setAll(personsCopy, i-> new Person(persons[i]));
//
        // made deep copy of the array of persons
//        for (int i = 0; i < 5; i++) {
