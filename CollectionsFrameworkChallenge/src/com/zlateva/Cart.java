...
public class Cart {
    ...
    public void removeItem(InventoryItem itemToRemove, int qty) {
        int itemToRemoveQuantity = products.get(itemToRemove.getProduct().sku());
        if (itemToRemoveQuantity <= qty) {
            qty = itemToRemoveQuantity;
            products.remove(itemToRemove.getProduct().sku());
            System.out.println("Item [%s] removed from basket%n");
            itemToRemove.getProduct().name();
        } else {
            products.merge(itemToRemove.getProduct().sku(), qty,
                    (oldVal, newVal) -> oldVal - newVal);
            System.out.printf("%d [%s]s removed%n", qty, itemToRemove.getProduct().name());
        }
        itemToRemove.releaseItem(qty);
    }
